...
void check4Tie();
void turnAroundTime( vector<jobs> v);
void waitingTime( vector<jobs> v);
...
void emitVector(const vector<jobs> v)
{
    cout << endl
         << "Job Chart" << endl
         << "Ar=Arrival, Pr=Priority, CT=cpu ime, CL=cpu ime Left, CD=cpu time completed, NN=exit time unit" << endl
...
void checkArrivals(vector<jobs> v, int t)
{
    ...
    for (int i = 0; i < v.size(); i++)
    {
        if (v.at(i).arrival == t)
        {
            cout << "process: " << v.at(i).idNum << " arrives" << endl;
            temp = v.at(i);
            if (!isEmpty())
            {
                if (temp.priority < PQ.top().priority)
                {
                    cout << "Context switch"
                         << endl
                         << "process " << preemptJobNum << " preempts" << endl;
                }
            }
            PQ.push(temp);
        }
...
void check4Preempt(int tempID)
{
    cout << "process: " << tempID << " preempts" << endl;
}
...
void waitingTime(vector<jobs> v)
{
   ...
   for (int i = 0; i< v.size(); i++)
   {
       float jobWT = v.at(i).nightNight - v.at(i).arrival - v.at(i).cpuTime;
       avgWT += jobWT;
       cout << "process " << i << " WT: "
            << v.at(i).nightNight << "  - " <<v.at(i).arrival << " - " << v.at(i).cpuTime << " = " << jobWT << endl;
   }
   cout << "-----------------------------\n"
        << "          AVG WT:" << avgWT / v.size()
        << "\n----------------------------\n"  << endl;
...
void check4Exit(int t, int bigT, vector<jobs> &v)
{
    if (!isEmpty())
    {
        ...
        {
            PQ.pop();
            cout << "process: " << tempID << " night night" << endl;
            for (int i = 0; i < v.size(); i++)
            {
